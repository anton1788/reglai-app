  ...
  const handleAIAssistant = (message) => {
    ...
    setTimeout(() => {
      ...
      if (userRole === 'master' || userRole === 'foreman') {
        if (lowerMsg.includes('мои заявк')) {
          response = `📋 Ваши заявки:\n• Всего: ${metrics.myAppsCount}\n• В работе: ${metrics.myActiveApps}\n• Завершено: ${metrics.myAppsCount - metrics.myActiveApps}\n\n💡 Создайте новую заявку через кнопку "+"`;
        } else if (lowerMsg.includes('объект')) {
          const myObjects = [...new Set(applications?.filter(a => a.user_id === user?.id).map(a => a.object_name) || [])];
          response = `🏗️ Ваши объекты:\n${myObjects.length > 0 ? myObjects.map(o => `• ${o}`).join('\n') : 'Нет объектов'}`;
        } else {
          response = `👷 Привет, мастер! Я могу помочь с:\n• "мои заявки" — список ваших заявок\n• "объекты" — ваши объекты`;
        }
      }
          ...
          <div className="p-4 space-y-4">
            <div className="flex flex-wrap gap-2">
              {userRole === 'master' || userRole === 'foreman' ? (
                <>
                  <button onClick={() => handleAIAssistant('мои заявки')} className="px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg text-xs">📋 Мои заявки</button>
                  <button onClick={() => handleAIAssistant('объекты')} className="px-3 py-1.5 bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 rounded-lg text-xs">🏗️ Объекты</button>
                </>
