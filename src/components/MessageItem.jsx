...
const MessageItem = memo(({ 
  ...
  const getUserRole = () => {
    const companyUser = companyUsers?.find(u => u.user_id === msg.user_id);
    return companyUser?.role || msg.user?.user_metadata?.role;
  };
  ...
  return (
    <div 
      id={`msg-${msg.id}`}
      className={`group flex gap-3 ${isOwn ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-300 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 rounded-lg transition-all -mx-2 px-2 py-1`}
    ...
    >
      ...
      <div className={`max-w-[85%] md:max-w-[75%] ${isOwn ? 'order-1' : ''}`}>
        ...
        {!isOwn && !isDeleted && (
          <div className="flex items-center gap-2 mb-1 ml-1">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">
              {getUserRole() === 'super_admin' ? 'Admin' : getUserRole()}
            </span>
        ...
        {/* Пузырь сообщения */}
        <div className={`relative rounded-2xl px-4 py-2.5 shadow-sm transition-all message-bubble ${
          isOwn 
        ...
        {/* Действия над сообщением (как в Telegram) */}
        <div className={`flex items-center gap-1 mt-1 transition-opacity ${showActions && !isDeleted && !isEditing ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          ...
          <div className="relative">
            ...
            {showReactionsPicker === msg.id && (
              <div className="absolute bottom-full left-0 mb-2 p-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 flex flex-wrap gap-1 z-50 animate-in fade-in zoom-in duration-200">
                  ...
                  return (
                    <button
                      ...
                      className={`p-2 rounded-lg transition-all text-xl ${
                        hasReacted 
                          ? 'bg-[#4A6572]/10 scale-110 ring-2 ring-[#4A6572]/30' 
          ...
          {/* Кнопка сохранения в избранное (как в Telegram) */}
          <button 
          ...
          >
            {isSaved ? (
              <BookmarkCheck className="w-3.5 h-3.5 text-[#F9AA33]" />
            ) : (
            ...
            <div className="flex items-center gap-0.5 pl-1 border-l border-gray-200 dark:border-gray-600">
              {canEdit && (
                <button 
                  ...
                  className="p-1.5 hover:bg-blue-100/50 rounded-full transition-all text-blue-500"
                  title="Редактировать"
                >
              ...
              {canDelete && (
                <button 
                  ...
                  className="p-1.5 hover:bg-red-100/50 rounded-full transition-all text-red-500"
                  title="Удалить"
                >
              ...
              return (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-full text-xs font-medium flex items-center gap-1 transition-all ${
                    hasReacted 
                      ? 'bg-[#4A6572]/10 text-[#4A6572] dark:text-[#F9AA33] ring-1 ring-[#4A6572]/30' 
                      : 'bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600'
                  }`}
                >
                  {emoji} <span className="opacity-80 text-xs">{count}</span>
                </button>
              );
            })}
          </div>
