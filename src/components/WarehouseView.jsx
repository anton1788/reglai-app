...
// Компонент: Таблица материалов с кнопкой удаления (ДЛЯ ДЕСКТОПА)
...
const TransactionRow = memo(({ trans }) => {
    ...
    <div className="space-y-3">
      {displayItems.length === 0 ? (
        ...
        displayItems.map((item, idx) => {
          ...
          return (
            <div key={item.id || idx} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-lg border border-gray-100 dark:border-gray-700">
              <div className="flex justify-between items-start mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-gray-900 dark:text-white truncate">{item.name || '—'}</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {item.unit || 'шт'} • Приход: {formatNumber(item.totalIncome || 0)}
                  </p>
                  {viewMode === 'fromApplications' && item.object_name && (
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-1">
                      📍 {item.object_name} • 👤 {item.foreman_name}
                    </p>
                  )}
                </div>
                <div className="text-right ml-2">
                  <div className={`text-2xl font-bold ${isLow ? 'text-red-600' : isMedium ? 'text-yellow-600' : 'text-green-600'}`}>
                    {formatNumber(balance)}
                  </div>
                  <div className="text-[10px] text-gray-400">шт.</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className={`px-2 py-1 rounded-full text-[10px] font-semibold ${
                  isLow ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                  isMedium ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' :
                  'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300'
                }`}>
                  {isLow ? 'Мало' : isMedium ? 'Средне' : 'Хорошо'}
                </span>
                {viewMode === 'fromApplications' && requested > 0 && (
                  <span className="px-2 py-1 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                    Заявка: {formatNumber(requested)}
                  </span>
                )}
              </div>
              ...
              <div className="flex gap-2">
                {canEdit && viewMode === 'warehouse' && (
                  <button
                    onClick={() => adjustBalance(item)}
                    className="flex-1 py-2.5 bg-gray-100 dark:bg-gray-700 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300"
                  >
                    Корректировка
                  </button>
                )}
                {balance > 0 && canEdit && viewMode === 'warehouse' && (
                  <button
                    onClick={() => setTransferModal({ isOpen: true, item })}
                    className="flex-1 py-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-sm font-medium text-blue-700 dark:text-blue-300"
                  >
                    Выдать
                  </button>
                )}
                <button
                  ...
                  className="flex-1 py-2.5 bg-gray-50 dark:bg-gray-700/30 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300"
                >
                  История
                </button>
              </div>
    ...
    <div className="max-w-7xl mx-auto p-3 space-y-4 pb-24"> {/* pb-24 чтобы нижняя навигация не перекрывала */}
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('warehouse') || 'Склад'}</h1>
        ...
        {canEdit && viewMode === 'warehouse' && (
          <button
            ...
            className="p-2.5 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-xl shadow-lg shadow-indigo-500/25"
          >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 -mx-4 px-4 no-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        <button
          ...
          className={`flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap ${
            showTransactions ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600'
          }`}
        >
        ...
        <button
          ...
          className="flex-shrink-0 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap bg-green-500 text-white hover:bg-green-600 shadow-md shadow-green-500/20 disabled:opacity-50"
        >
      ...
      {canEdit && (
        <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-200/60 dark:border-gray-700/60 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Автозаказ</span>
          </div>
          <button
            onClick={onToggleAutoReorder}
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${autoReorderEnabled ? 'bg-[#4A6572]' : 'bg-gray-300 dark:bg-gray-600'}`}
          >
            <span className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${autoReorderEnabled ? 'translate-x-5' : 'translate-x-1'}`} />
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <StatCard label="Мало" value={stats.lowStock} icon={AlertCircle} color="text-red-600" onClick={() => loadStatsDetails('lowStock')} />
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-11 pr-10 py-3 border border-gray-300 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
        />
        {searchTerm && (
